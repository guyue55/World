    ...
    <section className="relative overflow-hidden rounded-[2rem] border border-ink/10 bg-white/50 p-8 shadow-soft md:p-10">
      ...
      <div className="relative grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-4 text-5xl font-semibold leading-tight md:text-6xl">精选路径</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          {audiences.map((item) => (
            <div key={item.audience} className="rounded-2xl bg-paper/70 p-4">
              <p className="text-sm text-ink/50">{item.audience}</p>
              <p className="mt-1 text-2xl font-semibold">{item.count} 条路径</p>
            </div>
          ))}
        </div>
      </div>
