...
export function SceneIdentityBand() {
  ...
  return (
    <section
      ...
      className={`world-container pt-8 ${pathname === '/' ? 'pb-2' : ''}`}
    >
      <div className={`overflow-hidden rounded-[1.35rem] border p-4 backdrop-blur md:p-5 ${getToneClass(personality)}`}>
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="min-w-0">
            <p className={`flex items-center gap-2 text-xs font-semibold tracking-[0.28em] ${mutedTextClass}`}>
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center lg:justify-end">
            <div className="flex flex-wrap gap-2">
              {signals.map((signal) => (
                <span key={signal} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${chipClass}`}>
                  {signal}
                </span>
              ))}
            </div>
