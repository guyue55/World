  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[1fr_0.95fr]">
      <div className="rounded-[2rem] border border-white/65 bg-white/76 p-7 shadow-soft backdrop-blur md:p-8">
        ...
        <h2 className="mt-3 text-3xl font-semibold text-ink">反迷路罗盘</h2>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {metrics.map((metric) => (
            <div key={metric.label} className="rounded-2xl border border-white/10 bg-white/8 p-4">
              <p className="text-3xl font-semibold text-paper">{metric.value}</p>
              <p className="mt-1 text-sm font-semibold text-paper/82">{metric.label}</p>
              <p className="mt-1 text-xs leading-5 text-paper/55">{metric.note}</p>
            </div>
          ))}
        </div>
        {firstPath && (
