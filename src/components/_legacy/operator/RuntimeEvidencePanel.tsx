import { getRuntimeEvidenceRecordPlan } from '@/lib/phase-eleven-runtime'

export function RuntimeEvidencePanel() {
  const evidence = getRuntimeEvidenceRecordPlan()

  return (
    <section className="rounded-world border border-ink/10 bg-white/45 p-6 shadow-soft">
      <h2 className="text-2xl font-semibold">运行态证据记录</h2>
      <p className="mt-2 text-sm text-ink/55">evidenceReady: {String(evidence.evidenceReady)}</p>
