...
推荐入口是 `release:local-rc`：它会执行 RC 快检、lint、typecheck、强制 fresh build、构建产物时效验证、本地运行时 HTTP smoke、局域网浏览器 smoke、审计报告，并生成 `docs/90-archive/reports/worldos-local-rc-summary-report.json`。`check:rc:full` 保留为兼容别名。
