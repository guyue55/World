...
const worldRuntimeState = read('src/lib/world-runtime-state.ts')
for (const token of [
  'buildWorldRuntimeState',
  'getWorldRuntimeStateSummary',
  'getMotionRuntimeMode',
  'getSensoryRuntimeMode',
  'getAiRuntimeMode',
  'getPermissionRuntimeMode',
  'getSceneNavigationContract',
  'fallbackState',
  'sceneState',
  'transitionState',
  'motionMode',
  ...
  assert(worldRuntimeState.includes(token), `world-runtime-state 缺少 M1 运行态契约：${token}`)
}
for (const token of ['booting', 'firstVisit', 'sceneReady', 'transitioning', 'settled', 'degraded', 'error']) {
  assert(worldRuntimeState.includes(token), `world-runtime-state 缺少 scene state：${token}`)
}
for (const token of ['full', 'reduced', 'off', 'quiet', 'silent', 'public', 'authorized', 'denied', 'local', 'lan', 'offline-like']) {
  assert(worldRuntimeState.includes(token), `world-runtime-state 缺少运行边界：${token}`)
}
assert(!/window|document|localStorage|sessionStorage/.test(worldRuntimeState), 'world-runtime-state 必须保持纯函数，不得访问浏览器状态')

const runtimeProvider = read('src/components/world/WorldRuntimeProvider.tsx')
for (const token of ['buildWorldRuntimeState', 'sceneRuntime', 'sceneState', 'transitionState', 'motionMode', 'sensoryMode', "networkMode: 'local'"]) {
  assert(runtimeProvider.includes(token), `WorldRuntimeProvider 缺少统一运行态接入：${token}`)
}

const transitionShell = read('src/components/world/SceneTransitionShell.tsx')
for (const token of ['data-scene-state', 'data-transition-state', 'data-motion-mode', 'data-sensory-mode', 'data-permission-mode', 'data-network-mode']) {
  assert(transitionShell.includes(token), `SceneTransitionShell 缺少运行态 evidence：${token}`)
}

const statusPage = read('src/app/status/page.tsx')
for (const token of ['getPublicSceneSummary', 'getWorldRuntimeStateSummary', 'SceneRuntimeStatusPanel', 'sceneRuntimeSummary', 'runtimeStateSummary']) {
...
for (const token of ['场景运行时', 'Scene Registry', 'World Runtime State', '本地 / 局域网', '不读取私密层', 'reduced-motion']) {
