...
import { assertV9ServiceBoundary } from '../src/features/v9-service-platform'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/v9-service-platform/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing V9 batch ${batch}`]

  const data = readJson(`data/v9-service-platform/batches/${hit}`)
  ...
  if (data.serviceLive !== false) errors.push(`batch ${batch} serviceLive must be false`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/v9-service-platform/stages/stage-${String(stage).padStart(2, '0')}.json`
  ...
  if (!exists(stageFile)) errors.push(`missing stage file ${stageFile}`)
    ...
    if (data.serviceLive !== false) errors.push(`stage ${stage} serviceLive must be false`)
  }
...
export function checkBoundary() {
  return assertV9ServiceBoundary()
}
