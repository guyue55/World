...
import { assertR2WorldExperienceBoundary } from '../src/features/r2-world-experience'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/r2-world-experience/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing R2 batch ${batch}`]

  const data = readJson(`data/r2-world-experience/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/r2-world-experience/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertR2WorldExperienceBoundary()
}
