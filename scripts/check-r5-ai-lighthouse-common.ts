...
import { assertR5AiLighthouseBoundary } from '../src/features/r5-ai-lighthouse'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/r5-ai-lighthouse/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing R5 batch ${batch}`]

  const data = readJson(`data/r5-ai-lighthouse/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/r5-ai-lighthouse/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertR5AiLighthouseBoundary()
}
