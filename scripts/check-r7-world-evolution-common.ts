...
import { assertR7EvolutionBoundary } from '../src/features/r7-world-evolution'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/r7-world-evolution/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing R7 batch ${batch}`]

  const data = readJson(`data/r7-world-evolution/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/r7-world-evolution/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertR7EvolutionBoundary()
}
