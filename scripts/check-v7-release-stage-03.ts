import { checkBoundary, checkRequiredFiles, checkStage, failIfErrors } from './check-v7-release-common'

const errors = [
  ...checkStage(3, [9,10,11,12]),
  ...checkRequiredFiles([
  'data/v7-release-ops/roadmap.json',
  'data/v7-release-ops/extension-registry.json',
  'src/features/v7-release-ops/index.ts',
  'src/app/v7-release/page.tsx',
  'docs/10-development-history/v7-release-ops/stage-03-report.md',
  'data/v7-release-ops/operations-cadence.json',
  'data/v7-release-ops/backup-rollback.json',
  ]),
  ...checkBoundary(),
]

failIfErrors(errors)
console.log('V7 release stage 03 check passed.')
