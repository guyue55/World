...
import { assertR1ProductionBoundary } from '../src/features/r1-production-stabilization'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/r1-production-stabilization/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing R1 batch ${batch}`]

  const data = readJson(`data/r1-production-stabilization/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/r1-production-stabilization/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertR1ProductionBoundary()
}
