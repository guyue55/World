...
assert(sensoryAudioRegistry.name === 'WorldOS 场景声景注册表', 'Sensory Audio Registry 名称不正确')
...
for (const token of ['worldDayPeriod', 'worldSeason']) assert(provider.includes(token), `WorldRuntimeProvider 缺少环境属性：${token}`)
for (const token of ["data-world-day-period='day'", "data-world-season='winter'"]) assert(globalStyles.includes(token), `全局场景样式缺少环境选择器：${token}`)
assert(!fs.existsSync(rel('src/components/world/RuntimeAtmosphere.tsx')), '不得恢复覆盖所有场景的 RuntimeAtmosphere 视觉层')
...
const soundscapeEngine = read('src/lib/runtime/soundscape-engine.ts')
for (const token of ['AudioContext', 'visibilitychange', 'dispose', 'setScene']) assert(soundscapeEngine.includes(token), `SoundscapeEngine 缺少能力：${token}`)
