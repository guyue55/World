...
import { assertR8PublicOperationsBoundary } from '../src/features/r8-public-operations'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/r8-public-operations/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing R8 batch ${batch}`]

  const data = readJson(`data/r8-public-operations/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/r8-public-operations/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertR8PublicOperationsBoundary()
}
