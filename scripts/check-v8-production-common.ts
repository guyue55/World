...
import { assertV8ProductionBoundary } from '../src/features/v8-production-ops'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/v8-production-ops/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing V8 batch ${batch}`]

  const data = readJson(`data/v8-production-ops/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/v8-production-ops/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertV8ProductionBoundary()
}
