...
import { assertV10IntelligentWorldBoundary } from '../src/features/v10-intelligent-world'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/v10-intelligent-world/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing V10 batch ${batch}`]

  const data = readJson(`data/v10-intelligent-world/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/v10-intelligent-world/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertV10IntelligentWorldBoundary()
}
