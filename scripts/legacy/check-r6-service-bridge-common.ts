...
import { assertR6ServiceBoundary } from '../src/features/r6-service-bridge'
...
export function checkBatch(batch: number) {
  const batchDir = path.join(process.cwd(), 'data/r6-service-bridge/batches')
  const files = fs.readdirSync(batchDir)
  const hit = files.find((file) => file.startsWith(String(batch).padStart(2, '0') + '-'))
  const errors: string[] = []

  if (!hit) return [`missing R6 batch ${batch}`]

  const data = readJson(`data/r6-service-bridge/batches/${hit}`)
...
export function checkStage(stage: number, batches: number[]) {
  const errors: string[] = []
  const stageFile = `data/r6-service-bridge/stages/stage-${String(stage).padStart(2, '0')}.json`
...
export function checkBoundary() {
  return assertR6ServiceBoundary()
}
