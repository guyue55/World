...
- [ ] **B.1** 从仓库已有真实内容中选择一个公开代表节点和一次可追溯的小更新；不得编造作者经历，也不得用 RC / QA / WorldOS 自述节点。
...
- [ ] **C.1** 写 Clock 失败测试：时区、dayProgress、dayPeriod、seasonProgress、边界、hidden 恢复和确定性 worldDateKey；实现纯 `buildWorldTimeSnapshot()`。
