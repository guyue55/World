...
当前 Goal 默认自动交付 `LOCAL_LIVING_WORLD_CANDIDATE_AI_FALLBACK_HUMAN_AUDIO_PENDING`：

- localhost 与 LAN IP 可运行。
- 公开世界七场景具有持续、可暂停、各不相同的生命。
- 同一公开事实源支持静态和动态两种投影。
- 主要迁移、内容生长、回访、声音与低光灯塔形成闭环。
- 事实可以导出、校验并在临时工作区恢复。
...
- 实时 AI 陪伴质量，除非合法 Provider 在当前环境真实存在并通过评测。
...
1. 执行计划 A-H 全部勾选，每项有账本记录、命令、证据和中文提交。
...
5. 时间 / 季节、内容更新、回访、声音、AI fallback、权限、导出恢复和资源失败形成闭环。
...
`LOCAL_LIVING_WORLD_CANDIDATE_AI_FALLBACK` 与 `LOCAL_LIVING_WORLD_CANDIDATE_AI_PROVIDER` 仅是未来晋级状态，必须由用户在 Goal 外核验真实人类听感、Provider 与证据并批准新的控制版本；当前 Goal 不得自行进入。
