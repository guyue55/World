...
| 动态表现 | 已有 Scene Runtime、首次进入、转场、空气层、旅程记忆、Scene QA、SceneWorldPortal | 动态地基成立，但场景差异和连续探索仍需加强 |
| 视觉观感 | Phase 33/34 已从博客 Hero 推进为场景门户，并收口移动端首屏 | 已改善“博客骨架”，但还需要更多场景级行为 |
| 权限边界 | owner/auth/API 边界已有服务端守门思路 | 必须继续坚持后端控制权限，前端只做体现 |
| 验收链路 | `release:local-rc` 已成为本地可信入口 | 后续每个大里程碑都必须回归本地 RC |
| 外部上线 | 继续冻结 | 不作为当前目标 |
...
| M2 | 信息架构收束 | 首页、Atlas、Timeline、Archive、Paths、Node、Ask、Status 职责清晰 | 部分完成 |
| M3 | 场景化体验闭环 | 首次进入、场景门户、转场、空气层、旅程记忆形成闭环 | 已起步，需增强场景差异 |
| M4 | 内容生命系统 | 节点、关系、区域、生命周期、推荐路径统一驱动各入口 | 待重点推进 |
| M5 | 探索体验增强 | 访问者能沿路径连续探索，知道下一步和返回哪里 | 部分完成 |
| M6 | AI 灯塔只读导览 | AI 只解释、问路、推荐，不改数据、不越权 | 待推进 |
| M7 | Owner/权限边界成熟 | 本地 Owner 只读工作台、API 边界、私密排除可信 | 部分完成 |
| M8 | 性能与轻量化 | 动态体验不拖慢首屏，移动端和 reduced-motion 稳定 | 持续门禁 |
