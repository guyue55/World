...
| 动态表现 | 已有 Scene Runtime、首次进入、转场、空气层、旅程记忆、Scene QA、SceneWorldPortal；首页首屏恢复动态世界运行态脉冲层；5 条核心转场语义进入注册表和状态页 | 动态地基成立，已不再只是静态博客外壳；下一步是继续提升细节质感 |
| 视觉观感 | Phase 33/34 已从博客 Hero 推进为场景门户，并收口移动端首屏；Atlas 从完整列表改为星图和代表节点预览 | 已改善“博客骨架”，后续重点是更丰富的抵达、穿行、回望微交互 |
...
| M3 | 场景化体验闭环 | 首次进入、场景门户、转场、空气层、旅程记忆形成闭环 | 已完成本地/LAN 基础闭环，持续打磨细节 |
