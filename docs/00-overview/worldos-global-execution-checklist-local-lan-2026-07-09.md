...
- [ ] 确认首页只承担“进入世界/继续旅程”，不再承载过多说明。
- [ ] 确认 Archive 承担检索归档，Paths 承担旅程，不互相抢功能。
...
- [ ] Atlas：强化星图穹顶和区域关系，而不是普通卡片列表。
- [ ] Timeline：强化时间河、阶段流、事件展开。
- [ ] Archive：强化档案馆、卷宗、检索和保存感。
- [ ] Paths：强化旅程板、起点、进度、下一站。
- [ ] Node：强化节点房间、来源场景、关系出口。
...
- [ ] 定义公开节点最低字段：标题、摘要、区域、关系、生命周期、推荐路径。
- [ ] 明确精选内容准入：无摘要、无关系、无区域不得进入精选。
- [ ] 确认内容事实源被 Atlas、Timeline、Archive、Paths、Node 共用。
- [ ] 为关系增加“为什么相关”的中文说明。
- [ ] 检查内容字段是否存在重复模型或脚本复写。
...
- [ ] 用户可以从首页进入路径，连续读到第三个节点而不迷路。
- [ ] 不需要理解项目背景也能开始。
- [ ] localStorage 不保存权限判断或私密内容。

## 5. 后续主线：只读智能与可信维护

### H. AI 灯塔只读导览

- [ ] 定义 public context slice。
- [ ] 后端组织公开上下文。
- [ ] 前端只展示后端返回，不硬编码权限。
- [ ] Provider disabled 时提供 dry-run 和中文解释。
- [ ] AI 输出必须带来源或推荐路径。
- [ ] 不允许 AI 修改节点、路径、权限或发布状态。

验收：

- [ ] 未授权不能读取 owner/private。
- [ ] `/ask` 能清楚说明 AI 当前能力边界。
- [ ] `npm run check:api-boundary` 通过。

### I. Owner 与权限边界

- [ ] Owner 摘要 API 保持服务端 guard。
- [ ] `/status` 只显示可公开或 owner 允许的摘要。
- [ ] 私密档案、vault、owner-only 继续后端控制。
- [ ] 前端显隐基于服务端事实源。
- [ ] 权限扫描覆盖 token、owner、private、vault 关键词。
...
- [ ] `/status` 汇总本地 RC、Scene QA、内容质量、audit、权限边界。
- [ ] 用中文解释状态，不只展示内部字段。
- [ ] 明确外部发布冻结原因。
- [ ] 报告链接指向归档证据。
...
- [ ] 普通维护者能看懂当前能不能继续推进。
- [ ] 不泄露 token、私密原文、owner-only 细节。

## 6. 横向主线：性能、轻量化、可持续

### K. 性能预算

- [ ] 建立当前 First Load JS 和主路由体积基线。
- [ ] 新增动效不得引入新大型依赖。
- [ ] GSAP 保持按需加载或局部使用。
...
- [ ] `npm run build:production-ci` 无异常增长。
- [ ] `npm run release:local-rc` 通过。
- [ ] 移动端截图无明显遮挡和横向溢出。

### L. 验收产物策略

- [ ] 明确哪些报告提交，哪些报告只作为临时证据。
- [ ] 对只更新时间戳的报告做避免无意义 diff 的策略。
- [ ] Scene QA、LAN RC、local RC summary 的职责清晰。
- [ ] 每次大里程碑只保留必要截图证据。
